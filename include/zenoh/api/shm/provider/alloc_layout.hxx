...
class AllocLayout : public Owned<::z_owned_alloc_layout_t> {
   public:
    ...
    AllocLayout(const ShmProvider& owner_provider, std::size_t size, ZResult* err = nullptr)
        : Owned(nullptr) {
        __ZENOH_RESULT_CHECK(::z_alloc_layout_new(&this->_0, interop::as_loaned_c_ptr(owner_provider), size),
    ...
    AllocLayout(const ShmProvider& owner_provider, std::size_t size, AllocAlignment alignment, ZResult* err = nullptr)
        : Owned(nullptr) {
        __ZENOH_RESULT_CHECK(::z_alloc_layout_with_alignment_new(&this->_0, interop::as_loaned_c_ptr(owner_provider), size, alignment),
                             err, "Failed to create SHM Alloc Layout");
    }
